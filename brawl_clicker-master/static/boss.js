...
// Игра 2: Аэрохоккей (заменяем на предоставленный код с улучшениями)
    ...
    let puckSpeedX = 7; // Скорость из текущей игры
    let puckSpeedY = 7; // Скорость из текущей игры
    ...
    const maxSpeed = 20; // Добавляем максимальную скорость
    const minSpeed = 3; // Добавляем минимальную скорость
    const speedBoost = 1.1; // Добавляем ускорение после отскока
    ...
    function startGame2() {
        ...
        puckSpeedX = 7 * (Math.random() > 0.5 ? 1 : -1); // Скорость из текущей игры
        puckSpeedY = 7 * (Math.random() > 0.5 ? 1 : -1); // Скорость из текущей игры
    ...
    function resetPuck() {
        puckX = gameContainer2.offsetWidth / 2 - 10;
        puckY = gameContainer2.offsetHeight / 2 - 10;
        puckSpeedX = 7 * (Math.random() > 0.5 ? 1 : -1) * (1 + level * 0.1); // Скорость из текущей игры
        puckSpeedY = 7 * (Math.random() > 0.5 ? 1 : -1) * (1 + level * 0.1); // Скорость из текущей игры
        lastPaddleHit = null;
    }
