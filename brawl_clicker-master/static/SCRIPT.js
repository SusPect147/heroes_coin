...
let totalCoins=0;
currentScoreElement.textContent=totalCoins;
window.coinsPerPoint=1;
let coinsPerPointLevel=1;
...
const energyCost=10;
window.energyRecoveryRate=5;
window.coinsPerClick=1;
let minigamesPlayed=0;
let isSubscribedToTelegram=true;
...
let currentScore=parseInt(localStorage.getItem('currentScore')) || 5000;
...
function updateAutoClickerDetails() {
    ...
    coinElement1.textContent=upgradeCost;
} else {
coinElement1.textContent="Максимум";
}
}
const coinsPerClick=autoClickerLevel*10;
...
function startAutoClicker() {
    ...
    setInterval(()=> {
    if (autoClickerEnabled) {
    currentScore+=autoClickerLevel*10;
...
const itemDetails=document.querySelector('.item_up2 .item-details');
const coinElement=document.querySelector('.coin_up');
...
itemDetails.textContent=`${window.coinsPerClick} • Уровень ${level}`;
if (level < maxLevel) {
...
level++;
coinsPerClick=level;
window.updateCoinsPerClick(coinsPerClick);
localStorage.setItem('level', level);
window.updateScore(currentScore-currentPrice);
updateLevelDetails();
...
const upgradeMessage3=document.createElement('div');
...
const upgradePricesEnergy=[2000, 5000, 15000, 30000, 60000];
let energyLevel=parseInt(localStorage.getItem('energyLevel'), 10) || 1;
const maxEnergyLevel=upgradePricesEnergy.length;
...
function restoreEnergy() {
    ...
    localStorage.setItem('currentEnergy', window.energy);
    updateEnergyDetails();
}
updateEnergyDetails();
...
localStorage.setItem('currentEnergy', 0);
localStorage.setItem('maxEnergy', 100);
localStorage.setItem('energyLevel', 1);
document.addEventListener('DOMContentLoaded', ()=> {
...
const upgradeMessage4=document.createElement('div');
const itemDetails4=document.querySelector('.item_up4 .item-details_4');
const coinElement4=document.querySelector('.coin_up4');
const currentScoreElements=document.querySelectorAll('.currentScore');
...
const upgradePrices=[1000, 5000, 20000, 100000];
...
function updateUpgradeCostDisplay() {
    const cost=recoveryLevel < maxRecoveryLevel ? calculateUpgradeCost(recoveryLevel):'Максимум';
    if (coinElement4) {
...
localStorage.setItem('recoveryLevel', 1);
localStorage.setItem('energyRecoveryRate', baseRecoveryRate);
recoveryLevel=1;
...
localStorage.setItem('energyRecoveryRate', energyRecoveryRate);
